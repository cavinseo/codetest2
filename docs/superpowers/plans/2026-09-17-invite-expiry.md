...
- 2026-09-17. 사용자의 운영 오류 신고에 따라 미사용 초대와 같은 이메일·프로그램의 승인된 일반 멘티가 따로 존재하는 경우를 보완한다. 다른 초대 연결이 없는 해당 멘티만 관리자 날짜 저장 시 연결하고 기한을 함께 저장한다. 이 경우는 위 표의 기존 별도 계정 일괄 거절에서 제외한다. 상세 원인·검증은 `task-legacy-link-fix.md`에 기록한다.
