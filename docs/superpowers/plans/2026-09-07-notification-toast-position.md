...
### Task 3: 두 테마 모두 녹색·붉은 바탕에 흰 글씨로 고정

**요구:** 알림은 녹색 바탕에 흰 글씨, 경고는 붉은 바탕에 흰 글씨.

**현황:** Task 2 가 라이트 모드에서만 연한 바탕 + 진한 글씨로 갈라 두었다
(`.light .toast-success` = 연녹색 바탕 + 진초록 글씨). 대비 자체는 충분했지만
같은 알림이 테마에 따라 다른 모양이 된다.

- [x] **Step 1: `.light .toast-*` 분기 3개를 없앤다** — `app/globals.css`

남는 규칙은 `.toast-success`(#047857 바탕·흰 글씨), `.toast-error`(#b91c1c 바탕·흰
글씨), `.toast-info`(#1d4ed8 바탕·흰 글씨) 뿐이고 두 테마에 그대로 적용된다.
흰 글자 대비는 녹색 5.5:1, 붉은색 6.5:1 로 WCAG AA 를 넘는다.

라이트 모드에서 이 규칙이 포괄 규칙에 덮이지 않는지 다시 확인했다 — 토스트에는
`text-<색>-` 유틸리티가 없고 `text-sm` 만 있어 `.light [class*="text-…"]` 규칙들이
매치되지 않는다.

- [x] **Step 2: 두 테마를 브라우저로 대조한다** (정적 목업 렌더)
- [ ] **Step 3: 게이트** — 이 세션에서는 실행 불가. CI 로 확인한다.
