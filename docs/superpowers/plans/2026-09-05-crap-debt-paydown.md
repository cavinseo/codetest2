...
> **완결(2026-09-05)**: Task 1~4 전부 구현·감리 승인 완료. CRAP 위험 **12 → 0**(최대 306.0 → 29.5)이고, `main` 푸시마다 `--fail-over=30` 게이트가 자동으로 돈다. 판정 근거는 `docs/superpowers/reports/2026-09-05-crap-debt-paydown/` 의 보고서 4건과 각 Task 의 감리 기록에 있으며, **CRAP 수치는 전부 감리자가 워크플로를 직접 실행해 로그에서 읽은 값이다**(실행 AI 보고를 옮겨 적지 않았다).
...
> **감리 기록(2026-09-05) — 승인. 상환 완료.** 병합 `51d9471`, 작업 커밋 `5c71c2b`, 보고서 `40ceaf0`(`docs/superpowers/reports/2026-09-05-crap-debt-paydown/task-3.md`).
> 감리자가 직접 확인한 것: ① 경계 — Task 3 구간 변경이 테스트 4개(+160줄)·계획서 체크박스 2개·보고서뿐이고, 금지 경로(Task 1·2 가 넣은 두 테스트 포함) 전부 무변경. ② 계약 B 준수 — 병합이 문서 1개만 들여왔고 감리자의 Task 1·2 승인 기록이 브랜치에 보존됐다. ③ 포팅 충실도 — 네 파일 모두 원본과 `git diff` **빈 출력**, `it` 개수 **4 / 11 / 6 / 19**(계약값과 일치). ④ **CRAP 재측정을 감리자가 직접 실행** — [run #24](https://github.com/cavinseo/codetest2/actions/runs/33980340505) (`workflow_dispatch`, `40ceaf0`). **실행 결론 `success`** — `Build report` 단계가 처음으로 통과했다(`--fail-over=30`). 로그에서 읽은 값: **CRAP > 30 = 0**, "CRAP 이 30 을 넘는 함수는 없다.", 최대 CRAP **29.5**, 커버리지 100% 함수 **549(80%)**. ⑤ 뮤테이션 회귀 없음 — 살아남은 뮤턴트 목록이 기준선과 동일하다.
>
> **상환 경과(전부 감리자가 워크플로를 직접 돌려 읽은 값)**
>
> | 시점 | 위험(>30) | 최대 CRAP | 커버리지 100% 함수 | 실행 결론 |
> | --- | ---: | ---: | --- | --- |
> | 기준선 `985a47e` | 12 | 306.0 | 535 (78%) | failure |
> | Task 1 후 `e1bb433` | 9 | 132.0 | 540 (79%) | failure |
> | Task 2 후 `58feb28` | 4 | 56.0 | 545 (80%) | failure |
> | Task 3 후 `40ceaf0` | **0** | **29.5** | **549 (80%)** | **success** |
>
> 주의: 최대 CRAP 29.5 는 임계값 30 에 바짝 붙어 있다. 커버리지 없는 함수가 하나만 들어와도 게이트가 선다 — Task 4 의 무장이 그만큼 값어치가 있다는 뜻이자, 무장 직후 첫 실패가 새 부채의 신호이지 오작동이 아니라는 뜻이다.
...
> **감리 기록(2026-09-05) — 승인. 게이트 무장 완료.** 작업 커밋 `4e6a51d`, 보고서 `87c4d3b`(`docs/superpowers/reports/2026-09-05-crap-debt-paydown/task-4.md`). Task 1~3 은 병합 커밋 `b6f15e1` 로 `main` 에 들어갔다.
> 감리자가 직접 확인한 것: ① 경계 — `b6f15e1` 이후 변경이 `crap.yml`·기준선 문서·계획서 체크박스·보고서 넷뿐이고, `lib`·`app`·`components`·`tests`·`scripts`·패키지 파일·`ci.yml`·`prisma` 전부 무변경. ② `crap.yml` diff 가 **`on:` 블록 4줄과 그 위 주석 2줄뿐**이며 D-3 주석이 보존됐다. `--fail-over=30` 이 82행에 그대로 있다(변조 없음). ③ 기준선 문서에 지어낸 수치가 없다 — 적힌 값이 모두 감리자가 실행한 run 의 값과 일치한다. ④ **자동 실행 확인** — `main` 의 CRAP 실행 이력을 API 로 직접 읽었다:
>
> | run | 커밋 | event | 결론 |
> | --- | --- | --- | --- |
> | #21 | `985a47e` 기준선 | `workflow_dispatch` | failure (위험 12) |
> | #25 | `b6f15e1` 병합 | `workflow_dispatch` | success |
> | **#26** | `4e6a51d` 무장 | **`push`** | **success** |
> | #27 | `87c4d3b` 보고서 | **`push`** | success |
>
> #26 이 완료 판정의 핵심 근거이고, #27 은 요구하지 않은 추가 증거다 — 문서만 바꾼 커밋에서도 게이트가 돌았으므로 **특정 커밋에서 한 번 통과한 것이 아니라 `main` 푸시마다 상시로 걸린다**는 뜻이다.
> 실행 AI 가 오래된 로컬 `main` 의 미추적 측정 산출물을 임의로 지우지 않고 **중단·보고 후 승인받아** 처리한 것은 계약대로다(`.gitignore` 규칙이 Task 1 커밋에 있어 동기화 전에는 무시되지 않았다).

---

## 완결 요약 (2026-09-05)

| 시점 | 커밋 | 위험(>30) | 최대 CRAP | 커버리지 100% 함수 | 실행 | 결론 |
| --- | --- | ---: | ---: | --- | --- | --- |
| 기준선 | `985a47e` | 12 | 306.0 | 535 (78%) | #21 dispatch | failure |
| Task 1 | `e1bb433` | 9 | 132.0 | 540 (79%) | #22 dispatch | failure |
| Task 2 | `58feb28` | 4 | 56.0 | 545 (80%) | #23 dispatch | failure |
| Task 3 | `40ceaf0` | **0** | 29.5 | 549 (80%) | #24 dispatch | **success** |
| 병합 | `b6f15e1` | 0 | 29.5 | 549 (80%) | #25 dispatch | success |
| Task 4 | `4e6a51d` | 0 | 29.5 | 549 (80%) | **#26 push** | **success** |

`lib/**` 는 한 줄도 바뀌지 않았다 — 사라진 위험은 전부 커버리지가 생겨서 사라진 것이고 동작 변화가 없다. 뮤테이션 전체 점수 98.47% 와 살아남은 뮤턴트 15건 목록도 기준선과 같다(회귀 없음).

### 남은 부채 (이번 범위 밖)

- **뮤테이션 생존 15건** — `lib/import-cascade-guard.ts` 88.9%(7), `lib/invite-code.ts` 90.3%(7), `lib/import-json-schema.ts` 92.3%(1). 마지막 1건은 도구 한계로 판정된 등가 뮤턴트다.
- **주의 구간 17건**(15 < CRAP ≤ 30). 최대 **29.5** 로 임계값에 0.5 차이다. 커버리지 없는 함수가 하나만 들어와도 게이트가 선다 — **무장 직후 첫 실패는 오작동이 아니라 새 부채의 신호다.**
- **미병합 브랜치 `claude/carp-inspection-46phhc` 의 나머지 테스트 7개**(`oauth-nonce`, `product-attributes-utils`, `import-cascade-guard`, `api-spec-save`, `api-worksheet-cascade`, `signup-prefill`, `unsaved-changes`). 위험 12건과 무관해 제외했다. `oauth-nonce` 는 `main` 도 수정했고 lib 도 달라 포팅 시 충돌 가능.
- **브랜치 보호 미설정** — 이 게이트는 푸시 *이후* 검사다. 직접 푸시 자체를 막지는 않는다.
