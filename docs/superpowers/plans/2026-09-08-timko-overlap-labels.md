...
- 묶기는 순수 함수(`lib/timko-point-clusters.ts`)로 빼서 vitest + stryker 로 검증한다.
  ...
  검증으로 이월한다.
- 감리자가 아래 알고리즘을 node 로 돌려 확인한 것: 시나리오 8개 통과, 사용자 데이터 15개를
  WS-6(r=8)·WS-7(r=6) 좌표계에 넣으면 **묶음은 `4·7`, `11·15` 둘뿐**이고 나머지 11개는 단독
  이라 라벨은 모두 13개다.
...
5. **화면 검증(감리자·사용자, 실브라우저)**: WS-6 TIMKO 탭과 WS-7 탭 각각에서
   (a) 겹친 자리에 `4·7`, `11·15` 라벨이 보이고 나머지 11개 번호가 전부 보이는가(라벨 13개),
