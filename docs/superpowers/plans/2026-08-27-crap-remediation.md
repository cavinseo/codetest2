...
> **결정 (2026-08-31, 사용자):** (나)를 택하되 범위를 좁힌다.
>
> 17건을 한 덩어리로 적었으나 측정 결과를 다시 보니 두 건은 성격이 다르다.
> `import-cascade-guard.ts:65` 의 `', '` 는 경고 문장에서 항목을 잇는 구분자라,
> 없으면 "Kano 설문 응답 12건벤치마크 3건이 함께 삭제됩니다" 가 된다.
> `:27` 의 `EMPTY_CASCADE_IMPACT` 는 "영향 없음" 반환값의 모양 그 자체다.
> 둘 다 카피가 아니라 로직에 가깝고 단언 한 줄이면 잡힌다.
>
> 따라서 **문구 15건(`MEMBER_AI_MODE_LABELS`·`MEMBER_AI_MODE_DESCRIPTIONS`·
> `INVITE_CODE_MESSAGES`)은 제외하고, `import-cascade-guard.ts` 의 2건은 단언한다.**
