...
**Approach:** 두 화면의 기존 크기, 어두운 배경, 둥근 테두리, 하단 불만족 계수, 사분면 이름을 유지한다. 사용자 추가 요청에 따라 만족 계수의 축·눈금·제목은 우측에 표시한다. 공용 10×10 격자의 셀별 색상과 상단 3.2~5.0 눈금을 적용하고 기존 점 좌표와 선택 동작을 유지한다. 셀 불투명도는 0.65이며, 눈금·제목·사분면 이름은 흰색 굵은 글자로 표시하고 사분면 글자의 어두운 외곽선을 유지한다.
