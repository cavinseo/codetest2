...
**2026-09-03 게이트 통과.** 사용자가 로컬에서 돌렸다.
...
이 Task 는 테스트를 늘리지 않으므로 건수가 Task 2 와 같다. 즉 게이트 결과만으로는 이 Task 의 코드가 그 트리에 있었는지 가려낼 수 없다. `git log --oneline -1` 이 `00fb9ec` 를 가리켜 그 트리가 이 Task 의 커밋(`6ac6beb`)을 담고 있음을 확인했다. **테스트 건수가 늘지 않는 Task 는 게이트 출력만으로 검증을 확정하지 말고 HEAD 를 함께 확인해야 한다.**
