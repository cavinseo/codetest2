...
**Task F·G·H 이후 갱신된 항목:** V-3(리셋 문구가 적합도를 언급해야 한다),
V-9(WS-4 적합도 화면에 경고가 붙었다), V-9b·V-10~V-12(신규).
...
#### V-10. Google 미가입 계정에 가입을 권한다 (Task H)

**주의: 이 항목만 로그인 흐름이다.** 나머지는 워크시트다.

1. 로그아웃 상태에서 로그인 화면 → **Google 계정으로 로그인**
2. **이 서비스에 가입돼 있지 않은** Google 계정을 고른다

- **통과:** 오류 문구 대신 **제안 배너**가 뜨고, 거기에 **그 Google 계정 주소가
  그대로** 보인다. `[이 계정으로 회원가입]` 버튼이 있다
- **실패 A** — 예전처럼 문구만 뜬다 → 쿠키가 안 심겼거나 화면이 못 읽었다
- **실패 B** — 배너는 뜨는데 **주소가 다르거나 비어 있다** → 쿠키 파싱 문제.
  **보이는 주소를 그대로 적어 보내라**

#### V-11. 가입 화면에 이메일이 채워진다 (Task H)

1. V-10 에서 `[이 계정으로 회원가입]` 클릭

- **통과:** 이메일 칸이 **그 주소로 채워져 있고**, 아래에 "Google 계정에서
  가져왔습니다. 다른 주소로 가입하려면 고쳐 주세요."가 보인다
- **이어서 확인:** 그 칸을 **다른 주소로 고칠 수 있는가**. 읽기 전용이면 안 된다

#### V-12. 프리필이 특권이 아니다 (Task H — 가장 중요)

**이 항목이 이 변경의 안전성을 확인한다.**

1. V-11 상태에서 나머지 항목(이름·비밀번호·프로필)을 채우고 가입
2. 그 계정으로 **바로 로그인**을 시도

- **통과:** 가입 후 "관리자 승인 후 로그인할 수 있습니다" 안내가 뜨고,
  **로그인하면 `승인 대기 중` 으로 막힌다**
- **실패:** Google 로 확인된 계정이라고 **바로 로그인된다** → 승인 게이트가 뚫린 것이다.
  **즉시 중단하고 알려라**

**곁들여:** 로그인 화면을 새로 연 뒤(Google 을 거치지 않고) 가입 화면에 직접 들어가면
이메일 칸이 **비어 있어야** 한다. 쿠키는 한 번 쓰고 지운다.
...
V-9b WS-6 Kano 질문 화면 경고 (Task F)  [ ]
V-10 Google 미가입 → 가입 제안 배너     [ ]
V-11 가입 화면 이메일 프리필            [ ]
V-12 가입해도 승인 대기로 막힌다        [ ]   ← 핵심
...
| V-12 실패(바로 로그인됨) | **즉시 중단.** 승인 게이트가 뚫린 것이다. Task H 를 되돌린다 |
| V-10·V-11 실패 | Task H 배선 문제. 게이트는 통과했으므로 화면 쪽이다 |
