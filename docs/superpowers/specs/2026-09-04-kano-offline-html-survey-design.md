...
**가능하다. 스키마 변경·새 npm 의존성 없이 된다.** 서버가 자급자족 HTML(외부 리소스 0)을 내려주고, 피설문자가 오프라인에서 답한 뒤 "답변 파일 저장"으로 작은 JSON(`.kano.json`)을 받아 담당자에게 보내면, 담당자가 여러 파일을 한 번에 올려 응답 기록이 만들어진다. 응답 파일은 `requirementId` 와 질문 세트 해시를 담아 "어떤 문구에 대한 답인지"를 스스로 증명하며, 저장은 엑셀 경로와 같은 트랜잭션 함수를 공유한다.
...
[관리자]  WS-6 › 미리보기 › 「오프라인 HTML 받기」
      ...
      Kano_설문_<프로젝트>.html   ← 자급자족: 문항·라디오·인라인 CSS/JS, requirementId·질문 해시 내장
             │  메일·메신저·USB 로 전달
             ▼
[피설문자]  파일 더블클릭(file://) → 라디오 응답 → 「답변 파일 저장」
             │  모든 문항 필수 검사 → Blob + <a download>
             ▼
      kano-response-<id8>.kano.json   ← projectId, questionSetHash, questions[].h, submissionId, 이메일(선택), answers[requirementId]
             │  담당자에게 회신
             ▼
...
### 4.1 응답 파일 형식 (`format: "kano-offline-response"`, `version: 1`)
...
- 파일명은 **ASCII** 다(`kano-response-<submissionId 앞 8자>.kano.json`). 실증에서 POSIX 로케일 Chromium 이 비ASCII 파일명을 `download` 로 바꾸는 것이 확인됐다. 프로젝트명·응답자명은 파일명에 넣지 않는다.
...
| 응답 파일 파서 | `lib/kano-offline-response.ts` (신규, 순수) | 텍스트 → 검증된 응답 파일, 현재 질문 세트와 대조, 합성 이메일, 배치 내 중복 검사. |
...
- 「응답 파일로 업로드」 카드 아래에 **「오프라인 응답 파일 업로드」** 카드: `<input type="file" multiple accept=".json,.kano.json">`, 업로드 전 클라이언트 사전 검사(`File.text()` 로 `format`·`projectId` 확인 — 다른 프로젝트 파일·설문 HTML 오첨부를 서버 왕복 없이 표시), 결과 패널(성공 수·실패 파일과 사유). **추가(append) 고정** — 덮어쓰기(replace)는 제공하지 않는다.
...
| 피설문자가 답변 파일 대신 설문 HTML·스크린샷을 보냄 | 설계 B | 가드가 `.json` 만 받고 파서가 첫 글자 `<` 를 보면 "설문 파일이 아니라 답변 파일" 로 안내. 스크린샷·종이는 엑셀 전용 양식으로 손입력(기존 경로) |
...
**C. 온라인 우선·자기 저장형 HTML — 기각(후속 후보).** 인터넷이 되면 바로 제출하는 경험은 좋지만, 공개 엔드포인트가 임의 이메일로 `KanoSurveyInvitation` 을 선점해 이후 관리자의 온라인 초대(`invite/route.ts` 가 400)를 막는 부작용이 있고, 무상태 토큰은 회수가 안 된다(스키마 변경 필요). 접목한 것: 초대 내역 라벨링, 파일별 실패 수집(부분 성공), 토큰 `offline_<submissionId>` 로 DB 수준 멱등, `overwrittenRespondentCount` 표시. "HTML 이 자기 자신에 답을 써 넣어 저장"하는 단일 파일 왕복은 서버 파서가 HTML 섬을 읽기만 하면 되므로 v2 후보로 남긴다.
...
| # | 항목 | 결정 | 이유 |
| --- | --- | --- | --- |
| 1 | 응답 파일 형식 | 전용 JSON `.kano.json` (4.1절) | requirementId·해시로 자기 증명. 심사 만장일치 |
| 2 | 단일 파일 왕복(자기 저장형 HTML) | v1 에서는 하지 않는다. 서버 파서는 순수 JSON 만 받되, `<` 로 시작하는 파일은 "설문 파일" 로 안내한다 | PoC 로 증명된 경로를 먼저 내고, HTML 섬 읽기는 v2 |
...
| 7 | 파일명 | HTML `Kano_설문_<프로젝트명 정제>.html`(서버가 정함, RFC 5987) / 응답 `kano-response-<id8>.kano.json`(ASCII) | 실증 |
| 8 | 배치 정책 | 검증 통과 파일만 한 트랜잭션으로 저장, 실패 파일은 목록으로 반환. **요청당 20 파일·파일당 256 KB**, 화면이 20개씩 순차 전송, 라우트 `maxDuration = 60` | 운영자 마찰 최소 + Vercel·Prisma 상한 |
...
**사용자가 뒤집을 수 있는 제품 결정(기본값과 바꿀 때의 비용):** ① 별도 답변 파일 vs 자기 저장형 단일 HTML — 기본 별도 파일; 바꾸면 Task 3 렌더러와 Task 4 파서(HTML 섬 추출)가 커진다. ② 익명 허용 — 기본 허용; 금지하면 파서 한 줄·안내문. ③ 기존 응답자 덮어쓰기 — 기본 거절; 항상 덮어쓰기로 바꾸면 Task 5 의 충돌 검사 제거. ④ 배치 실패 정책 — 기본 부분 성공; 전부-또는-무로 바꾸면 Task 5 분기 하나. ⑤ 초대 만료 — 기본 즉시; +1년으로 바꾸면 리셋 후 온라인 응답 구멍이 생긴다. ⑥ 채널별 분석 — 기본 없음; 필요하면 `KanoResponse.source` 컬럼(원격 실DB 마이그레이션, 별도 계획). ⑦ 업로드 역할 확장 — 기본 없음.

### 9.1 이번 구현에서 빠지는 것

- zip 일괄 업로드(의존성 없음), 온라인 우선 제출, 자기 저장형 HTML, 파일 서명·암호화, 응답자 단위 삭제 API, 채널별 분석(출처 컬럼), 엑셀 라우트의 공용 함수 통합, 요구사항 저장 라우트의 캐스케이드 경고 보강, Safari·iOS·Android 검증(Task 0 파일럿과 실기동 단계로 이월).
