...
// ─── 프로그램으로 불러오기(프로젝트·멘티 공통) ──────────────────────────

/**
 * `already-here`  이미 이 프로그램 소속이다. 옮길 것이 없다.
 * `needs-confirm` 다른 프로그램 소속이라, 옮기기 전에 한 번 더 확인받아야 한다.
 * `ok`            어디에도 안 속해 있거나 확인을 받았다. 옮겨도 된다.
 */
export type ProgramMoveOutcome = 'already-here' | 'needs-confirm' | 'ok';

/**
 * 무언가를 이 프로그램으로 불러올 때 무엇을 해야 하는지 정한다.
 * 프로젝트 불러오기와 멘티 배정이 같은 규칙을 쓴다.
 *
 * 이미 **다른** 프로그램에 속해 있으면 조용히 가로채지 않고 한 번 멈춘다
 * (needs-confirm). 어디에도 안 속해 있으면(current === null) 빼앗는 것이
 * 아니므로 그냥 통과시킨다 — 확인을 받을 상대가 없다.
 *
 * Project.programId 는 NOT NULL 이라 프로젝트는 null 갈래를 타지 않는다.
 * User.programId 는 nullable 이라 멘티는 탄다(초대 코드 없이 가입한 멘티).
 */
export function programMoveOutcome(
    currentProgramId: string | null,
    targetProgramId: string,
    confirmed: boolean
): ProgramMoveOutcome {
    if (currentProgramId === targetProgramId) return 'already-here';
    if (currentProgramId === null) return 'ok';
