...
export const FUNCTION_ANALYSIS_PROMPT_VERSION = 'FAST-WS2-1.1';
...
- 보고서 제목은 <h1>, 아래 8개 절은 각각 <section> 안의 <h2>로 표시한다. 제목의 번호와 문구를 그대로 유지하고 설명은 <p>, 검토 항목은 <ul> 또는 <ol>로 작성한다. 이 프롬프트의 ### 표기는 절 정의이며 결과에 Markdown 제목으로 출력하지 않는다.
...
이번 요청의 산출물은 위 8개 절과 분석표를 포함한 단일 HTML 보고서다. 빈 양식이나 HTML 작성 방법 설명으로 대신하지 말고 입력 자료에 대한 실제 분석 내용을 채운다. 도구를 실행하지 않았다면 검증 스크립트가 통과했다고 쓰지 않는다.
`;
}
