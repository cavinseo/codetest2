...
export default function SalesTable({ projectId }: Props) {
    ...
    const addRow = () => setRows(prev => [...prev, { id: `new_${Date.now()}`, customer: '', amount: 0, competitor: '', order: prev.length }]);
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            if (res.ok) {
                ...
                setRows(data.rows.map((r: any) => ({
                    id: r.id, customer: r.customer ?? '', amount: r.amount ?? 0,
                    competitor: r.competitor ?? '', order: r.order,
                })));
                showToast('저장되었습니다.');
            } else {
        ...
        <div className="space-y-4 relative">
            ...
            <div className="flex items-center justify-between">
                <div>
                    ...
                    <p className="text-sm text-gray-500 mt-1">매출처별 매출 현황 (단위: 백만원)</p>
                </div>
                ...
                <div className="card border-rose-500/25 bg-rose-500/[0.04] animate-fade-in">
                    <div className="flex items-center justify-between gap-4">
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                <p className="text-white text-sm font-semibold">매출 데이터 초기화</p>
                ...
                <table className="w-full border-collapse text-sm">
                    <thead>
                        <tr className="bg-white/[0.04] border-b border-white/[0.08]">
                            <th className="border border-white/[0.06] p-3 text-gray-400 text-center w-[50px]">No.</th>
                            <th className="border border-white/[0.06] p-3 text-white font-semibold text-center">매출처</th>
                            <th className="border border-white/[0.06] p-3 text-amber-300 font-semibold text-center w-[180px]">매출액 (백만원)</th>
                            <th className="border border-white/[0.06] p-3 text-white font-semibold text-center">경쟁사명</th>
                    ...
                    <tbody>
                        {rows.length === 0 ? (
                            <tr>
                                <td colSpan={5} className="border border-white/[0.06] p-12 text-center">
                                    <p className="text-gray-500 text-sm mb-3">아직 데이터가 없습니다</p>
                            ...
                            rows.map((row, idx) => (
                                <tr key={row.id} className="border-b border-white/[0.04] hover:bg-white/[0.02] group">
                                    <td className="border border-white/[0.06] p-2 text-center text-gray-600 text-sm">{idx + 1}</td>
                                    <td className="border border-white/[0.06] p-0">
                                        <input type="text" value={row.customer} onChange={e => updateRow(row.id, 'customer', e.target.value)} className="w-full p-2.5 bg-transparent text-white text-sm border-none outline-none focus:ring-1 focus:ring-primary-500/50 placeholder-gray-700" placeholder="매출처명" />
                                    </td>
                                    <td className="border border-white/[0.06] p-0">
                                        <input type="number" value={row.amount || ''} onChange={e => updateRow(row.id, 'amount', parseFloat(e.target.value) || 0)} className="w-full p-2.5 bg-transparent text-amber-300 text-sm text-right font-mono border-none outline-none focus:ring-1 focus:ring-amber-500/50 placeholder-gray-700" placeholder="0" />
                                    </td>
                                    <td className="border border-white/[0.06] p-0">
                                        <input type="text" value={row.competitor} onChange={e => updateRow(row.id, 'competitor', e.target.value)} className="w-full p-2.5 bg-transparent text-white text-sm border-none outline-none focus:ring-1 focus:ring-primary-500/50 placeholder-gray-700" placeholder="경쟁사명" />
                                    </td>
                        ...
                        {rows.length > 0 && (
                            <tr className="bg-white/[0.03]">
                                <td colSpan={2} className="border border-white/[0.06] p-3 text-right text-white font-bold text-sm">합계</td>
                                <td className="border border-white/[0.06] p-3 text-right text-amber-300 font-bold font-mono">{totalAmount.toLocaleString()}</td>
