...
export default function WorksheetComments({ projectId, worksheetId }: { projectId: string; worksheetId: string }) {
    ...
    const load = useCallback(async () => {
        try {
            ...
            if (!res.ok) throw new Error(data.error || '코멘트를 불러오지 못했습니다.');
            setComments(data.comments); setCanComment(data.canComment); setUserId(data.userId);
        } catch (cause) { setError(cause instanceof Error ? cause.message : '코멘트 조회 실패'); }
    }, [url]);
    ...
    const save = async (method: string, id?: string) => {
        ...
        try {
            ...
            if (!res.ok) throw new Error(data.error || '본인 코멘트만 변경할 수 있습니다.');
            setText(''); setEditing(null); await load();
        } catch (cause) { setError(cause instanceof Error ? cause.message : '코멘트 저장 실패'); }
        finally { setBusy(false); }
    };
    return <section className="card mt-6 space-y-3" aria-label="워크시트 코멘트">
        <h2 className="font-semibold text-white">멘토 코멘트</h2>
        {error && <p role="alert" className="text-rose-400 text-sm">{error}</p>}
        {comments.length === 0 && <p className="text-sm text-gray-400">등록된 코멘트가 없습니다.</p>}
        {comments.map(comment => <article key={comment.id} className="border-b border-white/10 pb-3">
            ...
            {canComment && comment.authorId === userId && <div className="flex gap-3 mt-2">
                ...
                <button disabled={busy} className="text-sm text-rose-400" onClick={() => { if (window.confirm('이 코멘트를 삭제할까요?')) void save('DELETE', comment.id); }}>삭제</button>
            </div>}
        </article>)}
        {canComment && <form onSubmit={event => { event.preventDefault(); void save(editing ? 'PATCH' : 'POST', editing || undefined); }}>
            <label className="text-sm">{editing ? '코멘트 수정' : '코멘트 작성'}<textarea className="input mt-2" maxLength={5000} rows={3} value={text} onChange={event => setText(event.target.value)} disabled={busy} /></label>
            <button className="btn-primary mt-2" disabled={busy || !text.trim()}>{busy ? '저장 중…' : '코멘트 저장'}</button>
