...
import { GET, DELETE, PATCH } from '../../app/api/programs/[id]/route';
...
it('프로그램 수정은 실제 세션 권한을 적용하고 프로젝트·멘티 연결을 보존한다', async () => {
    const program = await seedProgram('editable');
    const mentee = await seedMentee('edit_mentee', program.id);
    await db.project.create({ data: { id: prefix + 'edit_project', name: '보존할 프로젝트', ownerId: mentee.id, programId: program.id } });
    const patchRequest = (userId: string, body: object) => new NextRequest(`http://localhost/api/programs/${program.id}`, {
        method: 'PATCH', headers: { 'Content-Type': 'application/json', cookie: `${SESSION_COOKIE_NAME}=${encodeSessionCookie({ userId, email: `${userId}@example.com`, name: userId })}` }, body: JSON.stringify(body),
    });
    const body = { name: '수정한 프로그램', organization: '변경 기관', startsAt: '2026-09-01', endsAt: '2099-12-31' };
    expect((await PATCH(patchRequest(managerId, body), params(program.id))).status).toBe(403);
    expect((await PATCH(patchRequest(mentee.id, body), params(program.id))).status).toBe(403);
    expect((await PATCH(patchRequest(adminId, body), params(program.id))).status).toBe(200);
    expect(await db.program.findUnique({ where: { id: program.id }, select: { name: true, organization: true, managerId: true, endsAt: true } })).toEqual({ name: body.name, organization: body.organization, managerId: adminId, endsAt: new Date(body.endsAt) });
    expect((await db.user.findUniqueOrThrow({ where: { id: mentee.id } })).programId).toBe(program.id);
    expect((await db.project.findUniqueOrThrow({ where: { id: prefix + 'edit_project' } })).programId).toBe(program.id);
    await db.program.update({ where: { id: program.id }, data: { managerId } });
    expect((await PATCH(patchRequest(managerId, { ...body, name: '담당 매니저 수정' }), params(program.id))).status).toBe(200);
});
