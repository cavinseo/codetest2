...
// 테스트를 모두 통과한 채로 살아 있었다. 이 파일은 POSTGRES_PRISMA_URL 이 가리키는 DB 에
// 실제로 행을 쓰고 지워서 그 부류의 버그를 잡는다.
//
// 반드시 테스트 전용 DB 를 가리키게 한 뒤 실행한다. POSTGRES_PRISMA_URL 이 실제로 테스트
// 전용 DB 를 가리키는지는 실행하는 사람이 확인해야 한다. 아래 안전장치는 그 값이 아예
// 비어 있는 경우만 막는다.
import { afterAll, afterEach, beforeAll, describe, expect, it } from 'vitest';
import { PrismaClient } from '@prisma/client';

// 안전장치: POSTGRES_PRISMA_URL 이 없으면 알아보기 힘든 연결 오류 대신 분명한 메시지로 멈춘다.
// 이 검사가 어떤 it()/beforeAll() 보다도 먼저, 모듈을 불러오는 시점에 동기적으로 실행되기
// 때문에 조건을 만족하지 못하면 PrismaClient 생성조차 되지 않고 파일 전체가 즉시 실패한다.
if (!process.env.POSTGRES_PRISMA_URL) {
    throw new Error(
        'POSTGRES_PRISMA_URL이 설정되어 있지 않다. 테스트 전용 로컬 Postgres를 가리키는 ' +
            'POSTGRES_PRISMA_URL을 설정한 뒤 다시 실행한다.'
    );
}
