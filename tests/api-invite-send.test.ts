...
describe('발송 가능한 코드와 이용 기한', () => {
    ...
    it('관리자가 연장한 회원은 최초 90일이 지나도 현재 이용 기한으로 발송한다', async () => {
    ...
    it('회원 기한이 없는 과거 코드는 저장된 이용 기간을 기준으로 만료시킨다', async () => {
        findUniqueInvite.mockResolvedValue({ ...usedInvite(), usedAt: future(-30), accessDurationDays: 30, usedBy: { ...linkedMentee(), accessExpiresAt: null } });
        expect((await POST(request(), context)).status).toBe(400);
        expect(sendMail).not.toHaveBeenCalled();
    });
