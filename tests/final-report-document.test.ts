...
it('omits missing market and customer paragraphs when a new product overview has been saved', () => {
    const legacyFree = { ...free, marketDefinition: '이전 시장', targetCustomer: '이전 고객' };
    const model = buildFinalReportModel({ ...overview, productName: overview.projectName }, data, legacyFree, []);
    expect(model.blocks).toEqual(expected);
});
